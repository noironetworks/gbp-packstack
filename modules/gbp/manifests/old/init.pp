...
#   }
   ...
   exec { "create_src_dir":
      ...
      require => [File['remove_src_dir'], Package['python-pip', 'python-pbr', 'git']],
   }
   ...
   exec {'get_apicapi':
      command => "/usr/bin/git clone https://github.com/noironetworks/apicapi.git $apicapidir",
      creates => $apicapidir,
      require => Exec['create_src_dir'],
   }
   ...
   exec {'install_pyopflexagent':
      command => "/usr/bin/git checkout d84b19c; /usr/bin/python setup.py install",
      cwd => $pyopflexagent,
